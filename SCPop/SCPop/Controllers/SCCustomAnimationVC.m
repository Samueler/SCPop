...
@interface SCCustomAnimationVC ()

@property (nonatomic, strong) SCCAAnimationPop *pop;

@end

@implementation SCCustomAnimationVC
...
- (void)normalShowWithoutMaskColor {
    self.pop = [SCCAAnimationPop showPopOnTargetView:UIApplication.sharedApplication.keyWindow];
    self.pop.touchDismiss = YES;
    [self popAddObserver];
}

- (void)normalShowWithMaskColor {
    self.pop = [SCCAAnimationPop showPopOnTargetView:UIApplication.sharedApplication.keyWindow maskColor:[UIColor colorWithWhite:0 alpha:0.3]];
    self.pop.touchDismiss = YES;
    [self popAddObserver];
}
...
- (void)blockShowWithoutMaskColor {
    ...
    self.pop = [SCCAAnimationPop showPopOnTargetView:UIApplication.sharedApplication.keyWindow showCompleted:^(BOOL finished) {
        if (finished) {
...
- (void)blockShowWithMaskColor {
    __block typeof(self) weakSelf = self;
    self.pop = [SCCAAnimationPop showPopOnTargetView:UIApplication.sharedApplication.keyWindow maskColor:[UIColor colorWithWhite:0 alpha:0.3] showCompleted:^(BOOL finished) {
        if (finished) {
            NSLog(@"Show Animation Finished");
        }
    } dismissCompleted:^(BOOL finished) {
